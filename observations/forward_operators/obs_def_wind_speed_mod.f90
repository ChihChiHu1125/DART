...
real(r8) :: slp(ens_size)     ! surface pressure
...
! Surface pressure at this location - this calls the model_mod code.
call interpolate(state_handle, ens_size, location, QTY_SURFACE_PRESSURE, slp, this_istatus)
call track_status(ens_size, this_istatus, wspd, istatus, return_now)
if (return_now) return
...
!where (istatus ==0) wspd = exp( (slp-98500)/400 )  
